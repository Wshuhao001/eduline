	...
	class TopicAction extends BaseAction
	{
		...
		function index() {
			...
			$topiclist = $this->topic
							  ...
							  ->findPage(20);
			// 附加帖子分类
			foreach ($topiclist['data'] as &$val) {
				$val['replycount'] = M('comment')->where( array('table'=>'group_topic','row_id'=>$val['id'],'is_del'=>0))->count();
			}
...
// 	    				D('GroupUserCount')->addCount($post_info['uid'], 'bbs', $this->gid);
		...
		public function topic()
		{
			...
			//其他帖子
			$otherThread = M('group_topic')->field('id,title')->where(array('uid'=>$thread['uid'],'is_del'=>0))->limit(5)->findAll();
			//最新帖子
			$hotThread = M('group_topic')->limit(5)->where(array('is_del'=>0))->field('id,title')->order('addtime desc')->findAll();

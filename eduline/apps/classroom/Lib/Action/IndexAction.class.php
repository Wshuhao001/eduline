...
class IndexAction extends CommonAction {
    ...
    public function index() {
        ...
        $free_limit_list = M('zy_video')->where($map)->order('ctime DESC')->limit(3)->select();
        ...
        //加载热门问答
        $wenda = M('zy_wenda')->where('is_del = 0')->order('wd_comment_count desc , wd_browse_count desc')->limit('6')->findAll();
        ...
        $time = time();
        //根据销售最佳读取最佳讲师等信息
        $beVideos = M()->query("SELECT zv.`id` as video_id, zv.`video_title`,zt.`id` as teacher_id, zt.`name`,zt.`inro`,zt.`head_id` FROM `".C('DB_PREFIX')."zy_video` zv,`".C('DB_PREFIX')."zy_teacher` zt WHERE zv.teacher_id=zt.uid AND zt.is_del=0 and zv.`is_del`=0 AND `is_activity`=1 AND `uctime`>'$time' AND `listingtime`<'$time' and teacher_id >0 GROUP BY `teacher_id` ORDER BY `video_order_count` DESC  , `teacher_id` DESC  LIMIT 4");
        if(!$beVideos){
            $beVideos = M()->query("SELECT `id` as teacher_id,`name`,`inro`,`head_id` FROM `".C('DB_PREFIX')."zy_teacher` WHERE is_del=0 order by `id` DESC  LIMIT 4");
        }

...
class YoutufaceAction extends Action
{
...
//            $uploadRet = Youtu::delperson($data['uid']);
            ...
            if($uploadRet['errorcode'] != 0 && $uploadRet['errormsg'] != 'ok'){
                if($uploadRet['errormsg'] == ERROR_PERSON_EXISTED){
                    $errmess = '个体已存在，请重新更换账户注册';
                }
            }
        }
        if($errmess == ''){
            $re = saveFaceImage($pic);
//            $data['face_id'] = saveFaceImage($pic);//保存注册时的人脸saveFaceImage方法
//            $re = M('user')->where(['uid'=>$this->mid])->save($data);
            if($re){
                $errmess = '注册成功';
            }
        }
    ...
    public function faceverify(){
        ...
        if($errmess == '' && $uname != ''){
            // 使用本地图片
            $uploadRet = YouTu::faceverifybase64($pic, $data['uid']);
//            $uploadRet = Youtu::delperson($data['uid']);
            // 使用远程图片
            //$url = getAttachUrlByAttachId($attach);
            //$uploadRet = YouTu::newpersonurl(SITE_PATH.$url, '1', ['uname']);
            //$uploadRet = YouTu::newpersonurl('http://fun.youth.cn/yl24xs/201702/W020170227510976500117.png', '2', ['username']);
            if($uploadRet['errorcode'] != 0 && $uploadRet['errormsg'] != 'ok'){
                $errmess = $uploadRet['errorcode'].'人脸检测失败';
            }else if($uploadRet['ismatch'] == 'true'){
                $errmess ='true';
            }else{
                $errmess = '人脸验证失败';
            }

...
class AdminOrderAction extends AdministratorAction {
    ...
    public function _initialize() {
        ...
        $this->pageTab[] = array('title' => '直播订单', 'tabHash' => 'course', 'url' => U('classroom/AdminOrder/live'));
...
//        $this->pageTitle['course'] = '约课订单 - 交易记录';
    ...
    public function live() {
        ...
        //查询数据列表
        $listData =M('zy_order_live')->where($map)->order('ctime DESC,id DESC')->findPage();
        //整理数据列表
        foreach ($listData['data'] as &$val) {
            $val['ctime']   = friendlyDate($val['ctime']);
            $val['uid']     = getUserSpace($val['uid'], null, '_blank');
            $val['live_id'] = M('zy_live')->where('id='.$val['live_id'])->getField('title');
            $val['price']   = '<span style="color:red">￥' . $val['price'] . '</span>';
        }
        $this->displayList($listData);
    }

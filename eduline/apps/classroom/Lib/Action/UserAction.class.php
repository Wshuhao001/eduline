    ...
    */
    ...
    public function recharge(){
        ...
        if($data['vip_type'] > 0 && $data['vip_expire'] >= time() ){
        	$data['vip_type_txt'] = M('user_vip')->where('is_del=0 and sort=' . $data['vip_type'])->getField('title');
        } else {
        	$data['vip_type'] = 0;
        }
...
<p style=\"color:#666\">如果链接无法点击，请将链接复制到浏览器的地址栏中进行访问。</p>";
    ...
    public function getTeacherLive(){
        ...
        if($this->base_config['live_opt'] == 1){
            ...
            $data = M('live')->where("speaker=".$tid)->order('live_id desc')->findPage($limit);
            foreach($data['data'] as &$val){
                $val['id'] = $val['SDK_ID'];
                $val['title'] = $val['subject'];
                $val['url'] = U('live/Index/live_teacher',array('SDK_ID'=>$val["SDK_ID"]));
            }
		}else if($this->base_config['live_opt'] == 2){

		}else if($this->base_config['live_opt'] == 3){
            $data = M('zy_live')->where("uid=".$uid)->order('id desc')->findPage($limit);
            foreach($data['data'] as &$val){
    ...
    public function getTeacherFace(){
        $old_num_list="";
        $course_list=M("zy_teacher_course")->where(array('course_teacher'=>$this->mid,'is_del'=>0))->order('ctime desc')->findAll();
    ...
    public function dellive(){
        if($this->base_config['live_opt'] == 1){
            ...
            $res = json_decode(request_post($url, $data), true);
            if ($res['code'] == 0) {
                if (M('zy_live')->where('id=' . $data['id'] . ' and uid=' . $this->mid)->delete()) {
                    $return['status'] = 1;
                    $return['data'] = '删除成功';
                } else {
                    $return['status'] = 0;

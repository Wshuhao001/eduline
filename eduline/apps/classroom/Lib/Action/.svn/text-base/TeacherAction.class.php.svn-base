...
class TeacherAction extends CommonAction{
    ...
    public function getList(){
        ...
        if($reservation>0){
            $InSql="";
            $result=M("zy_teacher_schedule")->where(array('pid'=>array('IN',$reservation)))->findALL();
        ...
        if ($data['data']) {
             foreach ($data['data'] as $key => &$value) {
                ...
                $value["video"] = M('zy_video')->where('is_del=0 and teacher_id='.$value['uid'])->order('video_order_count desc')->field('id,video_title')->find();
            }
    ...
    public function view(){
        ...
        //教师视频
        $video_count=M("zy_video")->where("teacher_id=".$data['uid'])->findALL();
    ...
    public function getVideoList(){
        ...
        $data = M('zy_video')->where($where)->order('id desc')->findPage(12);

...
class AlbumAction extends CommonAction {
    ...
    public function watch() {
        ...
        if ($type == 1) { //课程
            ...
            $data[0]['user_vip_level'] = D('ZyRecharge','classroom')->getUserVipLevel($this->mid ,intval($data[0]["vip_level"]));
        ...
        if( floatval( $data[0]['mzprice']['price'] ) <= 0 || ( intval($data[0]['vip_level'] ) && $data[0]['user_vip_level'] ) )  {
        	$is_free = 1;
        }

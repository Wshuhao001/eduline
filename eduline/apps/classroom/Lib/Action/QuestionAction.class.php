...
class QuestionAction extends CommonAction
{
  ...
  //       }
		...
		if($i){
			...
			//发送系统消息
			$teacher_id = M('zy_video')->where('id='.$data['oid'])->getField('teacher_id');
			$s['uid']   = $teacher_id;
			$url  = U('classroom/Video/view', array('id'=>$data['oid']));
            $s['title'] = getUserName($this->mid)."向你提问了";
            $s['body']  = getUserName($this->mid)."向你提问了，点击查看<a href=\"{$url}\" target=\"_blank\" >$url</a>".'<p>'.$data['qst_description'].'</p>';
            $s['ctime'] = time();
            if($this->mid !== $teacher_id) {

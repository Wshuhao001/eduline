...
class AdminQuestionAction extends AdministratorAction
{
	...
	public function index(){
		...
		$this->pageKeyList = array('question_id','question_category_name','question_type_title','question_point','question_content','sort','question_status','question_insert_date','DOACTION');
	...
	private function _getData($limit = 20, $is_del){
		if(isset($_POST)){
			$_POST['question_category'] && $map['question_category_id'] = intval($_POST['question_category']);
    ...
    public function doQuestionImport(){
		...
		for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
			if($i>2 && $data->sheets[0]['cells'][$i]){
				...
				$question_category     = $data->sheets[0]['cells'][$i][1];
				$question_type         = $data->sheets[0]['cells'][$i][2];
				...
				$question_qsn_guide    = $data->sheets[0]['cells'][$i][8] ? "暂无解析!" : $data->sheets[0]['cells'][$i][7];
				...
				if($question_type=M("ex_question_type")->where("question_type_id='".$question_type."'")->find()){
					...
					if( !M('ex_question')->where( $where )->find() ) {

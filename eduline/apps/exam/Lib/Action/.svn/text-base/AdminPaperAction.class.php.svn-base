...
class AdminPaperAction extends AdministratorAction
{
	...
	public function index(){
		...
		$this->pageKeyList = array('paper_id','paper_name','paper_describe','paper_type','paper_point','paper_question_count','paper_status','uname','paper_insert_date','DOACTION');
		...
		$category = M('ex_paper_category')->getField('paper_category_id,paper_category_name');
	...
	public function doAddPaper(){
		...
		if($post['paper_id']){
			if ($question_type) {
				$paper_question=D("ExPaper")->doPaperQuestion($question_type,$post['paper_id'] , $this->mid);
			...
			$paper_question=$question_type ? D("ExPaper")->doPaperQuestion($question_type,$result , $this->mid) : 0 ;
			unset($data);
			$data["paper_question_count"]=$paper_question["count"];
			$data["paper_point"]=$paper_question["score"];
	...
	public function getQuestionLists(){
    	...
    	if($_POST['question_category']) {
    		$map['question_category'] = intval( $_POST['question_category'] );
    	}
	...
	private function _getData($limit = 20, $is_del){
		if(isset($_POST)){
			...
			$_POST['paper_category'] && $map['paper_category_id'] = intval($_POST['paper_category']);
		}

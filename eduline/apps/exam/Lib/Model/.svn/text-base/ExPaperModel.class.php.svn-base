...
class ExPaperModel extends Model{
    ...
    public function getPaper($id){
    	...
    	$content_list=$this->join(C('DB_PREFIX')."ex_question q ON pc.paper_content_questionid=q.question_id ")->order("sort asc")->where("q.question_is_del = 0 and pc.paper_content_paperid=".$id)->field($field)->select();
    ...
    public function doPaperQuestion($question_type,$paper_id , $uid){

...
class AlbumAction extends Action
{
	...
	public function view() {
        ...
        if (!$data) {
            ...
            echo "<script>alert('专辑不存在');history.go(-1);</script>";
        }
        //处理数据
        ...
        //是否收藏
        $data['iscollect'] = D('ZyCollection',"classroom")->isCollect($data['id'], 'zy_album', intval($this->mid));
    ...
    public function buyOperating() {
        ...
        if (!$_POST['id']) {
            exit(json_encode(array('status'=>'0','info'=>'没有选择专辑!')));
        }
        if (isBuyAlbum($this->mid, $_POST['id'])) {
            exit(json_encode(array('status'=>'0','info'=>'您已经买了本专辑!')));
        }
        $album = D("ZyAlbum","classroom")->getAlbumById($_POST['id']);
        $albumId = intval($_POST['id']);
        $video_ids = trim(D("ZyAlbum","classroom")->getVideoId(intval($_POST['id'])), ',');
        ...
        if ($illegal_count > 0) {
            exit(json_encode(array('status'=>'0','info'=>'专辑中包含有过期的课程，无法整辑购买!')));
        }
        ...
        //添加消费记录
        M('ZyLearnc',"classroom")->addFlow($this->mid, 0, $total_price, $note = '购买专辑<' . $album['album_title'] . '>', $pay_result['rid'], 'zy_order_album');
       //添加订单记录到课程
        ...
        if(!empty($insert_value)){
            ...
            if ($table->query($query) !== false || $total_price == 0) {
                exit(json_encode(array('status'=>'0','info'=>'购买专辑成功!')));
                foreach ($album_info as $key => $video) {
            ...
            $albumname= D("ZyAlbum","classroom")->getAlbumTitleById($_POST['id']);
            $s['uid']=$this->mid;
            $s['title'] = "恭喜您购买专辑成功";
            $s['body'] = "恭喜您成功购买专辑：《".$albumname."》";
            $s['ctime'] = time();
            model('Notify')->sendMessage($s);
            exit(json_encode(array('status'=>'1','info'=>'购买专辑成功!')));
        }
    }
}
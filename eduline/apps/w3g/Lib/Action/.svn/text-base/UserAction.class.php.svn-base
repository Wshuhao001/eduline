...
class UserAction extends CommonAction
{
	...
	public function index() {
        ...
        $this->assign("tmp",model('Follow')->getFollowCount(array($this->mid)) );
		$this->display();
	}
    //用户设置
    ...
    */
    ...
    public function rechargeVip(){
        ...
        if($_POST['pay'] == 'alipay'){
            ...
            if($res){
                $this->assign('url',$res);
                $html = $this->fetch('wxpay');
                echo json_encode(['status'=>1,'data'=>['html'=>$html,'trade_no'=>$id]]);exit;
            }
        }
    }
    ...
    protected function wxpay($data){
        ...
        if($data){
            ...
            $body = isset($data['subject']) ? $data['subject'] :"eduline-充值中心";

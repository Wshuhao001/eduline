...
class MessageModel extends Model {
     ...
     * @param $fid 被评论人的id
     * @param $app_id 应用的id
     * @param int $app_uid 应用作者的uid
     * @param $app_table 评论所储存的表
     * @param int $to_comment_id 被评论的id
     * @param string $info
     * @param $to_comment
     */
    public function doCommentmsg($uid,$fid,$app_id,$app_uid=0,$app_table,$comment_id=0,$to_comment_id=0,$info="",$to_comment){
        $data=array(
            'uid'=>$uid,
            'app_id'=>$app_id,
            'fid'=>$fid,
     ...
     */
    public function _parseMessageList(&$list, $current_uid) {
        foreach ($list as &$v) {
    ...
    public function getMessageByListId($list_id, $uid, $since_id = null, $max_id = null, $count = 20) {
        ...
        foreach($res['data'] as $r_d_k => $r_d_v) {
            ...
            $res['data'][$r_d_k]['mtime'] = friendlyDate($r_d_v['mtime']);
            // 处理附件信息
            $attachType = '';
            $attachArr = array();

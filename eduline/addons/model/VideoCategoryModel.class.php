...
/**
	...
	public function getCategoryList($pid = -1) {
		$map = array();
		$pid != -1 && $map['pid'] = $pid;
		$data = $this->where($map)->order('`sort` ASC')->findAll(); 
		return $data;
	}
...
/**
	...
	public function getCategoryById($id) {
		...
		if(!empty($id)) {
			...
			$result = $this->where($map)->order('`sort` ASC')->find();
		}
	...
	public function getChildCategoryById($id){
		...
		if(!empty($id)){
			...
			$next_name = $this->where('zy_video_category_id = '.$id)->field('next_name')->order('`sort` ASC')->find();
...
/**
	...
	public function getNetworkList($pid = '0') {
		// 子地区树形结构
		if($pid != 0) {
			return $this->_MakeTree($pid,'0');
		}
		// 全部地区树形结构
//		$list = S('VideoCategory');
//		if(empty($list)) {
//			set_time_limit(0);
			$list = $this->_MakeTree($pid,'0');
...
//		}
	...
	public function getNetworkSList($pid = '0'){
		//地区-学校树形结构
		if($pid != 0){
			return $this->_MakeSTree($pid,'0');
		}
		$list = $this->_MakeSTree($pid,'0');
		return $list;
	}
...
/**
	...
	private function _MakeTree($pid, $level = '0') {
		...
		if($result) {
			foreach($result as $key => $value) {
				...
				$list[$id]['level'] = $level;
				$list[$id]['child'] = $this->_MakeTree($value['zy_video_category_id'], $level + 1);
	...
	private function _MakeSTree($pid,$level = '0'){
		$result = M('zy_school_category')->where('pid='.$pid)->order('`sort` ASC')->findAll();
		if($result) {
			foreach($result as $key => $value) {
				...
				$list[$id]['title'] = $value['title'];
				$list[$id]['level'] = $level;
				$list[$id]['child'] = $this->_MakeSTree($value['zy_school_category_id'], $level + 1);
			}
		}
...
//		$myIds = $myIds?implode(',',$myIds):0;
	...
	private function _getPids($_ids,&$myIds=array()){
		$ids  = array();
		$pids = $this->where(array('pid'=>array('in',(string)$_ids)))->field('zy_video_category_id')->order('`sort` ASC')->select();

...
class VideoApi extends Api{
    ...
    public function videoList(){
        ...
        if ($this->data ['pType'] == 2 || $this->data ['pType'] == 1) {
            ...
            $ptWhere = "(is_tlimit=1 AND starttime<{$time} AND endtime>{$time} AND limit_discount{$oc}0.0)";
    ...
    public function videoInfo(){
        ...
        $data['playtime'] = D('ZyOrder', 'classroom')->getOrderInfo($data ['uid'],$id);
    ...
    public function getCatalog(){
        $id = intval($this->data['id']);//vid
        $uid = intval($this->data['uid']);
        $data = D('VideoSection' ,'classroom')->setTable('zy_video_section')->getNetworkList(0 , $id); 
        foreach($data as &$val){
            if($val['child']) {
                foreach($val['child'] as &$v) {
                    $v['video_address'] = M('zy_video_data')->where('status =1 and is_del=0 and id='.$v['cid'])->getField('video_address');
                }
            }
        }
        $this->exitJson($data);
	...
	public function getBuyVideosList() {
		...
		$data = M ( 'ZyOrder' )->join ( "{$vtablename} on {$otablename}.`video_id`={$vtablename}.`id`" )->where ( $where )->group("{$vtablename}.`id`")->field ( $fields )->limit ( $this->_limit () )->select ();
		foreach ( $data as &$val ) {
	...
	public function getCollectVideoList(){
		...
		//取数据
		$data = M('ZyCollection')->join("{$vtablename} on {$ctablename}.`source_id`={$vtablename}.`id`")->where($where)->group("{$vtablename}.`id`")->field($fields)->limit($this->_limit())->select();
        //循环计算课程价格
        foreach($data as &$val){
	...
	public function doreviewvote(){
        ...
        if($type>0){
            ...
            if($i){
                $this->exitJson( array() ,0,"已取消");
            }else{
               $this->exitJson( array() ,10018,'取消失败！'); 
            }
        }else{
            //投票
            ...
            if($i){
                $this->exitJson( array() ,0,"点赞成功");
            }else{
               $this->exitJson( array() ,10018,'点赞失败'); 
            }
        }
    }

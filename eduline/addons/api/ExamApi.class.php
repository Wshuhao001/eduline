...
class ExamApi extends Api{
    ...
    public function getCategory(){
        $all  = array(0=>array( 'exam_category_id'=>'0','exam_category_name'=>'全部') );
        $list = M('ex_exam_category')->field("exam_category_id,exam_category_name")->findAll();
    ...
    public function getExamInfo(){
        ...
        $user_exam=D("user_exam")->where($where)->select();
        $data["count"]=count($user_exam);
    ...
    public function doUserExam(){
        ...
        $data["user_total_date"]=$this->exam->getTime($this->data["total_date"],time());

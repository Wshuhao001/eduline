...
class WendaApi extends Api{
    ...
    public function getCate(){
        $all  = array(0=>array( 'zy_wenda_category_id'=>'0','title'=>'全部') );
    	$list = M('zy_wenda_category')->order('sort asc')->findAll();
        $list = array_merge($all , $list);
        $list ? $this->exitJson($list , '1') : $this->exitJson( $all );
    }
    ...
    */
    ...
    public function getWendaByCourse(){
    	...
    	foreach($wendaList as &$val){
    		$val['uname']    = getUserName( $val['uid'] );
    		$val['userface'] = getUserFace( $val['uid'],'m' );
    		$val['ctime']    = date('Y-m-d',$val['ctime']);//格式化时间数据
    	}
	...
	public function strSearch(){
		...
		$wendalist=$this->wenda->where(" `wd_description` LIKE '%$str%'  ")->order('ctime ASC')->limit($this->_limit())->select();
		//格式化数据
		foreach($wendalist as &$val){
			$val['wd_comment'] = $this->wenda_comment->getNowWenda($val['id'],1);//取最新的一条评论
			$val['wd_comment']['userface']=getUserFace($val['wd_comment']['uid'],'m');
	...
	public function postWenda(){
        ...
        if( $type<=0 )$this->exitJson( array() ,10032,"对不起，发布类型错误！");
	...
	public function doWendaComment(){
        ...
        $cont = t($this->data['content']);//获取评论内容
        if( empty($wid) || empty($cont)) $this->exitJson( array() ,10033,"对不起，请检查数据是否完整");
        $data=array(
            'uid'=>$this->mid,
            'wid'=>$wid,
            'description'=>$cont,
	...
	public function detail(){
		...
		if($wdata){
			$wdata['ctime'] = date('Y-m-d',$wdata['ctime']);
			//问答浏览量+1
			M('zy_wenda')->where('id='.$wid)->setInc('wd_browse_count' ,1);
			$this->exitJson($wdata);
		}else{

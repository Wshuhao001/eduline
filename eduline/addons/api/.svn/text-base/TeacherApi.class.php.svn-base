...
class TeacherApi extends Api{
    ...
    public function getTeacherList(){
  		$teacherData = $this->teacher->where(array('is_del'=>0))->order('reservation_count DESC')->limit($this->_limit())->select();
  		if( !$teacherData ) {
        		$this->exitJson( array() );
  		}
          //计算每个教师的课程总数
  		foreach($teacherData as &$val){ 
  			$time  = time();
  			$where = "is_del=0 AND is_activity=1 AND uctime>$time AND listingtime<$time AND teacher_id= ".$val['uid'];
    ...
    public function searchTeacher(){
    	...
    	$map['is_del'] = 0;
    	$teacherData = $this->teacher->where($map)->order('reservation_count DESC')->limit($this->_limit())->select();
    	if( !$teacherData ) {
			   $this->exitJson( array() );
    	}
    	//计算每个教师的课程总数
    ...
    public function  getTeacher(){
        ...
        $teacherInfo = $this->teacher->where(array('uid'=>$teacherId,'is_del'=>0))->find();

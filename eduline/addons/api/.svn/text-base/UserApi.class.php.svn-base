...
class UserApi extends Api
{
    ...
    public function show()
    {
        ...
        $data['user_tag'] = !$data['user_tag'] ? '' : implode('、', $data['user_tag']);
        //关注情况
    ...
    {
        $this->user_id = !$this->user_id ? $this->mid : $this->user_id;
        $res = model('Follow')->getFriendsForApi($this->mid, $this->user_id, $this->since_id, $this->max_id, $this->count, $this->page);
        if ($res) {
            $this->exitJson($res);
        } else {
            $this->exitJson(array(), 10016, '你还没有好友');
        }
    }
    ...
    public function wap_search_user()
    {
        ...
        if ($userlist) {
            $this->exitJson($userlist);
        } else {
            $this->exitJson(array(), 10016, '没有搜索到相关用户');
        }
    }
...
//         }
     ...
     * @datetime 2017-01-19T15:15:23+080
     *
     * @version  1.0
     */
    public function pay()
    {
        ...
        if ($money <= 0) {
            $this->exitJson(array(), 10030, '充值金额不正确');
        }
        ...
        if (!in_array($this->data['pay_type'], ['alipay', 'wxpay'])) {
            $this->exitJson(array(), 10040, '请选择正确的支付方式');
        }
        $pay_data = array();
        if ($this->data['pay_type'] == 'alipay') {
            // 支付宝充值
            $pay_data['alipay'] = $this->alipay([
                'out_trade_no' => $id,
                'total_fee' => $money,
                'subject' => '学币充值',
            ]);
        } elseif ($this->data['pay_type'] == 'wxpay') {
            // 微信充值
            $pay_data['wxpay'] = (object) [];
        }
    ...
    protected function alipay($args)
    {
        ...
        $alipayClass->setConfig(array(
            ...
            'return_url' => '', // 页面同步跳转地址,表示支付完成后跳转的页面地址链接,APP支付没有跳转地址,由开发人员控制
        ));
        ...
        // 调用阿里的服务,调用api端支付
        $res = $alipayClass->goAliService('api');
        if ($res) {
            $this->exitJson($res, 1, '生成支付链接成功');
        } else {
            $this->exitJson(array(), 10040, '生成支付链接失败,请重新尝试');
        }
    }
    /**
     * 获取阿里支付的配置参数.
    ...
    public function show_user()
    {
        ...
        if(!$this->user_id){
            ...
            $data = model('User')->find($this->user_id);
        }
        if(!$data){
            $this->exitJson(array(),'10004','用户不存在');
        }
    ...
    //设置用户是否开启人脸识别
    public function set_user_face()
    {
        //获取当前用户uid
        if($this->mid){
            //查询当前用户数据
            $data = model('User')->find($this->mid);
            //设置人脸识别的状态 当未开启（为0）时，可开启；反之可关闭
            if($data['is_start'] == 0){
                $save['is_start']= 1;
                model('User')->where(['uid'=>$this->mid])->save($save);
                $this->exitJson(['login'=>$data['login'],'Messge'=>'已开启人脸识别']);
            }else if($data['is_start'] == 1){
                $save['is_start'] = 0;
                model('User')->where(['uid'=>$this->mid])->save($save);
                //echo M('user')->getLastSql();exit;
                $this->exitJson(['已关闭人脸识别']);
            }
        }
    }
    ...
    public function select_face()
    {
        ...
        if($this->mid){
            ...
            $cmp = $data['login'];
            //根据用户账户查询
            $is_start = model('User')->where(['login'=>$cmp])->getField('is_start');
            $this->exitJson(['login'=>$cmp,'is_start'=>(int)$is_start]);
        }
        //没登录返回
        $this->exitJson((object)[],0,'请先登录');
    }
}
